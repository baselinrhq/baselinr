...
ProfileMesh ships a first-class Dagster integration under `profilemesh.integrations.dagster`. It turns your profiling config into Dagster assets, optional sensors, and a ready-to-run job so teams can orchestrate profiling alongside their existing data pipelines.
...
The extra ensures `dagster`, `dagster-webserver`, and their dependencies are present. If you already manage Dagster dependencies yourself you can install plain `profilemesh` and rely on your environment’s Dagster packages.
...
CONFIG_PATH = Path(__file__).parent / "profilemesh.yml"
...
`build_profilemesh_definitions` wires up:

- A cached `ProfileMeshResource` so every asset shares the same parsed config.
- One asset per table pattern plus a summary asset.
- A pre-built asset job (`profilemesh_profile_all` by default).
- An optional `profilemesh_plan_sensor` that triggers runs when the plan changes.
...
- `profilemesh_plan_sensor` recalculates the profiling plan on each tick.
...
- Pass `enable_sensor=False` to `build_profilemesh_definitions` if you prefer manual or cron-triggered jobs. You can also instantiate the sensor directly via `profilemesh_plan_sensor` to customize intervals or force runs.

## Customization

`create_profiling_assets` accepts optional overrides:

- `asset_name_prefix`: defaults to `profilemesh`.
- `group_name`: defaults to `profilemesh_profiling`.
...
`build_profilemesh_definitions` forwards those overrides and supports toggling the sensor or supplying a different job name.
...
- If you rely on environment variables (`PROFILEMESH_*`) they’ll be resolved once per process thanks to the cached resource—restart Dagster when secrets change.
