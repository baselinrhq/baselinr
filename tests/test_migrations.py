...
def test_migrate_apply_sql(migration_manager, temp_db_engine):
    ...
    migration = Migration(
        ...
        up_sql="ALTER TABLE test_table ADD COLUMN name TEXT"
    )
    ...
    with temp_db_engine.connect() as conn:
        ...
        result = conn.execute(text(
            "SELECT version, description FROM profilemesh_schema_version"
        ))
        row = result.fetchone()
        assert row[0] == 1
...
def test_migrate_dry_run(migration_manager):
    ...
    migration = Migration(
        version=1,
        description="Test migration",
        up_sql="CREATE TABLE should_not_exist (id INTEGER)"
    )
...
def test_multiple_migrations_sequence(migration_manager, temp_db_engine):
    ...
    migration1 = Migration(
        version=1,
        description="Add column1",
        up_sql="ALTER TABLE test_table ADD COLUMN col1 TEXT"
    )
    
    migration2 = Migration(
        version=2,
        description="Add column2",
        up_sql="ALTER TABLE test_table ADD COLUMN col2 TEXT"
    )
