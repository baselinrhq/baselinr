...
def sample_runs(temp_db_engine):
    ...
    with temp_db_engine.connect() as conn:
        ...
        # Insert sample runs
        runs = [
            ("run-1", "customers", "public", now - timedelta(days=1), "production", "completed", 1000, 10),
            ("run-2", "customers", "public", now - timedelta(days=2), "production", "completed", 990, 10),
            ("run-3", "orders", "public", now - timedelta(days=1), "production", "completed", 5000, 15),
            ("run-4", "orders", "public", now - timedelta(days=10), "staging", "failed", None, None),
        ]
...
def sample_drift(temp_db_engine):
    ...
    with temp_db_engine.connect() as conn:
        ...
        events = [
            ("event-1", "drift_detected", "customers", "email", "null_percent", 1.0, 2.5, 150.0, "high", now - timedelta(hours=1)),
            ("event-2", "drift_detected", "orders", "total", "mean", 100.0, 120.0, 20.0, "medium", now - timedelta(hours=2)),
            ("event-3", "drift_detected", "customers", "age", "mean", 35.0, 35.5, 1.4, "low", now - timedelta(days=5)),
        ]
...
def test_query_runs_combined_filters(query_client, sample_runs):
    ...
    assert len(runs) == 2
    assert all(
        run.dataset_name == "customers" and
...
def test_query_table_history_with_schema(query_client, sample_runs):
    ...
    history = query_client.query_table_history(
        "customers",
        schema_name="public"
    )
