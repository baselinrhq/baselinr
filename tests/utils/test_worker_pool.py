...
class TestExecutionConfig:
    ...
    def test_warehouse_limits(self):
        ...
        config = ExecutionConfig(
            max_workers=16,
            warehouse_limits={"snowflake": 20, "postgres": 8, "sqlite": 1}
        )
...
class TestBackwardCompatibility:
    ...
    def test_default_execution_config_no_worker_pool(self):
        ...
        config = ProfileMeshConfig(
            ...
            storage=StorageConfig(
                connection=ConnectionConfig(type="sqlite", database="test.db")
            )
        )
    ...
    def test_sequential_execution_no_worker_pool(self, mock_calc, mock_builder, mock_conn):
        ...
        from profilemesh.config.schema import (
            ProfileMeshConfig, ConnectionConfig, StorageConfig,
            ProfilingConfig, TablePattern
        )
        ...
        config = ProfileMeshConfig(
            ...
            profiling=ProfilingConfig(tables=[
                TablePattern(table="test1"),
                TablePattern(table="test2")
            ])
        )
    ...
    def test_parallel_execution_creates_worker_pool(self, mock_calc, mock_builder, mock_conn):
        ...
        from profilemesh.config.schema import (
            ProfileMeshConfig, ConnectionConfig, StorageConfig,
            ProfilingConfig, TablePattern, ExecutionConfig
        )
...
class TestWarehouseSpecificLimits:
    ...
    def test_sqlite_forces_sequential(self, mock_conn):
        ...
        from profilemesh.config.schema import (
            ProfileMeshConfig, ConnectionConfig, StorageConfig,
            ProfilingConfig, ExecutionConfig
        )
    ...
    def test_warehouse_limit_overrides_max_workers(self, mock_conn):
        ...
        config = ProfileMeshConfig(
            ...
            execution=ExecutionConfig(
                max_workers=16,
                warehouse_limits={"postgres": 8}  # Limit postgres to 8
            )
        )
