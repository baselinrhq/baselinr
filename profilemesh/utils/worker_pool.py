...
class WorkerPool:
    ...
    def _update_active_workers_metric(self):
        ...
        try:
            from .metrics import is_metrics_enabled, active_workers
    ...
    def _log_task_completion(self, task_name: str, success: bool, error: Optional[Exception] = None):
    ...
    def _record_batch_duration(self, batch_size: int, duration: float):
        """Record batch duration metric."""
        try:
            from .metrics import is_metrics_enabled, batch_duration_seconds
...
def profile_table_task(
    ...
    logger = logging.getLogger(__name__)
    table_name = f"{table_pattern.schema_}.{table_pattern.table}" if table_pattern.schema_ else table_pattern.table
