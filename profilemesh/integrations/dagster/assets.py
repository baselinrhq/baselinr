"""
Dagster asset factory + job helpers for ProfileMesh.

This module builds Dagster assets from a ProfileMesh configuration so that
...
if DAGSTER_AVAILABLE:
    ...
    class ProfileMeshResource(ConfigurableResource):
        """Dagster resource that caches the ProfileMesh configuration."""
    ...
    def create_profiling_assets(
        ...
        *,
        ...
        """
        Create Dagster assets that materialize ProfileMesh profiling runs.

        Args:
            config_path: Path to the ProfileMesh configuration file.
        ...
        base_tags = {"profilemesh/environment": config.environment}
        if default_tags:
    ...
    def _create_table_asset(
        *,
        ...
        @asset(
            name=asset_name,
            group_name=group_name,
            description=f"ProfileMesh profiling for {table_pattern.table}",
            required_resource_keys={"profilemesh"},
            tags=dict(tags),
        )
        def table_profiling_asset(context: AssetExecutionContext):
            """Materialize profiling results for one table."""

            resource: ProfileMeshResource = context.resources.profilemesh
            ...
            success_mat = AssetMaterialization(
                ...
                description=f"ProfileMesh run completed for {table_pattern.table}",
            )
    ...
    def _create_summary_asset(
        *,
        ...
        @asset(
            ...
            group_name=group_name,
            description="Summary of ProfileMesh profiling runs",
            required_resource_keys={"profilemesh"},
            tags=dict(tags),
        )
    ...
    def create_profiling_job(
        *,
        ...
        """
        Create a Dagster job that targets all ProfileMesh assets.
        """
        ...
        if not asset_keys:
            raise ValueError("No assets provided when building the ProfileMesh job.")
