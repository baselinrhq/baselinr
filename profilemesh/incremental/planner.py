...
from .change_detection import build_change_detector, ChangeSummary
...
class IncrementalPlanner:
    ...
    def __init__(
        ...
        self.change_detector = build_change_detector(config.source.type, self.connector, incremental_cfg)
    ...
    def _decide_for_table(self, table: TablePattern, now: datetime) -> TableRunDecision:
        ...
        if state and summary.snapshot_id and summary.snapshot_id == state.snapshot_id:
            ...
            self._emit_skip(table, reason, state)
            return TableRunDecision(table=table, action="skip", reason=reason, snapshot_id=summary.snapshot_id)
        ...
        if cost_decision:
            if cost_decision["should_run"]:
                ...
                action = cost_decision["action"]
                reason = cost_decision["reason"]
                self._emit_skip(table, reason, state, snapshot_id=summary.snapshot_id, action=action)
                return TableRunDecision(
                    table=table,
                    action=action,
                    reason=reason,
    ...
    def _is_due(self, state: Optional[TableState], now: datetime) -> bool:
        ...
        interval_minutes = min(max(interval_minutes, cfg.min_interval_minutes), cfg.max_interval_minutes)
    ...
    def _emit_skip(
        ...
        if self.event_bus:
            self.event_bus.emit(ProfilingSkipped.create(
                table=table.table,
