...
class RCACollectorConfig(BaseModel):
    ...
    dbt: bool = Field(True, description="Enable dbt run collector")
    ...
    airflow: bool = Field(False, description="Enable Airflow run collector")
...
class RCAConfig(BaseModel):
    ...
    lookback_window_hours: int = Field(24, ge=1, le=168, description="Time window for finding causes")
    max_depth: int = Field(5, ge=1, le=10, description="Maximum depth for lineage traversal")
    max_causes_to_return: int = Field(5, ge=1, le=20, description="Maximum probable causes to return")
    min_confidence_threshold: float = Field(
        0.3, ge=0.0, le=1.0, description="Minimum confidence to include a cause"
    )
    auto_analyze: bool = Field(
        True, description="Automatically analyze anomalies when detected"
    )
