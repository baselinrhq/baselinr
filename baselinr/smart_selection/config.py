...
class SmartSelectionCriteria(BaseModel):
    ...
    min_queries_per_day: float = Field(
        1.0, ge=0.0, description="Minimum average queries per day"
    )
...
class SmartSelectionRecommendations(BaseModel):
    ...
    include_suggested_checks: bool = Field(
        True, description="Include suggested profiling checks"
    )
...
class SmartSelectionAutoApply(BaseModel):
    ...
    skip_existing: bool = Field(
        True, description="Skip tables already in explicit configuration"
    )
...
class SmartSelectionConfig(BaseModel):
    ...
    mode: str = Field(
        "recommend",
        description="Selection mode: 'recommend' (generate suggestions), 'auto' (apply automatically), or 'disabled'",
    )
    ...
    cache_metadata: bool = Field(
        True, description="Cache metadata queries for performance"
    )
    cache_ttl_seconds: int = Field(
        3600, ge=60, description="TTL for metadata cache in seconds"
    )
