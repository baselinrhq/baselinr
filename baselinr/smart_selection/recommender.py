...
class RecommendationEngine:
    ...
    def generate_recommendations(
        ...
        if self.smart_config.mode == "auto":
            ...
            logger.info(
                f"Applied confidence threshold {threshold}: {len(scored_tables)} tables"
            )
        ...
        recommendations = [
            self._create_recommendation(score) for score in recommended_scores
        ]
    ...
    def save_recommendations(
        ...
        with open(output_file, "w") as f:
            # Add header comment
            f.write(f"# Baselinr Table Recommendations\n")
    ...
    def _create_exclusions(
        ...
        scored_ids = {
            (s.metadata.database, s.metadata.schema, s.metadata.table)
            for s in scored_tables
        }
        recommended_ids = {
            (s.metadata.database, s.metadata.schema, s.metadata.table)
            for s in recommended_scores
        }
    ...
    def _calculate_confidence_distribution(
        self, scored_tables: List[TableScore]
    ) -> Dict[str, int]:
        """
