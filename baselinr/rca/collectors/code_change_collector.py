...
class CodeChangeCollector:
    ...
    def _infer_affected_pipelines(self, changed_files: List[str]) -> List[str]:
        ...
        for file_path in changed_files:
            ...
            # Dagster
            if (
                "dagster" in file_lower or "assets/" in file_lower
            ) and file_lower.endswith(".py"):
    ...
    def _infer_deployment_type(self, changed_files: List[str]) -> str:
        ...
        for file_path in changed_files:
            ...
            # Schema changes
            if (
                "schema" in file_lower
                or "migration" in file_lower
                or file_lower.endswith(".ddl")
            ):
                has_schema = True
    ...
    def verify_github_signature(payload: bytes, signature: str, secret: str) -> bool:
        ...
        expected_signature = (
            "sha256="
            + hashlib.sha256(secret.encode() + payload).hexdigest()
        )
